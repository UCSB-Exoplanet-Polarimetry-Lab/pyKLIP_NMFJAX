...
def klip_math(sci, refs, numbasis, covar_psfs=None, model_sci=None, models_ref=None, spec_included=False, spec_from_model=False):
    ...
    if numbasis[0] is None:
        evals, evecs = la.eigh(covar_psfs, subset_by_value = (tot_basis-np.min([100,tot_basis-1]), tot_basis-1))
        ...
        evals, evecs = la.eigh(covar_psfs, subset_by_value = (int(tot_basis-max_basis), int(tot_basis-1)))
        evals = np.copy(evals[::-1])
        evecs = np.copy(evecs[:,::-1])
