...
def butterfly_rdi_imgs(imgs,KLs_butterfly,KLs_corona,centers,wvs_indices,IWA, numthreads=None, pool=None):
    """
    ...
    Inputs:
        ...
        KLs_butterfly: List of butterfly KL basis [(NKL1,ny,nx), ...].
            These modes will be centered and rotated based on butterfly direction.
        KLs_corona: List of coronagraph KL basis [(NKL2,ny,nx), ...].
    ...
    Output:
        out: array of shape (N,y,x) containing the subtracted images
    """
    # out = np.zeros(imgs.shape)
    # for k,(img,center,wvid) in enumerate(zip(imgs,centers,wvs_indices)):
    #     print(k)
    #     out[k,:,:] = butterfly_rdi_img(img,KLs_butterfly[wvid],KLs_corona[wvid],center,IWA)
    # return out
    if pool is None:
    ...
    tasks = [tpool.apply_async(butterfly_rdi_img, args=(img,KLs_butterfly[wvid],KLs_corona[wvid],center,IWA)) for img,center,wvid in zip(imgs,centers,wvs_indices)]

    #reform back into a giant array
    out = np.array([task.get() for task in tasks])
...
def get_butterfly_phase(im,center,IWA,w_coef = 0.05):
    ...
    im_cp[np.where((r_grid<2*IWA)|(r_grid>50))] = np.nan
    for l,th_bin in enumerate(th_bins):
        sector = np.where((th_grid>th_bin[0])&(th_grid<th_bin[1]))
        azimuthal_intensity[l] = np.nansum(im_cp[sector])
    aziprof_spl = splrep(th_bins_center,azimuthal_intensity,w=1/(w_coef*azimuthal_intensity))
    butterfly_phase = np.rad2deg(th_bins_center)[np.argmax(splev(th_bins_center,aziprof_spl))]
    return butterfly_phase

def butterfly_rdi_img(img,KLs_butterfly,KLs_corona,center,IWA):
    """
    ...
    butterfly_phase = get_butterfly_phase(img,center,IWA,w_coef = 0.05)
    ...
    # plt.show()
    im_vec = np.ravel(img)
    where_im = np.where(np.isfinite(im_vec))
    ...
    tmp_model = np.concatenate([KLs_butterfly_rot[:,where_im[0]],
                                ...
                                np.ones((1,np.size(where_im[0])))],axis=0)
