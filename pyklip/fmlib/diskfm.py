...
class DiskFM(NoFM):
    ...
    def fm_from_eigen(self,
        ...
        if self.save_basis is True:
            ...
            if mode == 'RDI':
                self.klparam_dict['isRDI'] = float64(1.)
            else:
                self.klparam_dict['isRDI'] = float64(0.)

            [IWA, OWA] = IOWA
            self.klparam_dict['IWA'] = float64(IWA)
            self.klparam_dict['OWA'] = float64(OWA)

            self.klparam_dict['input_img_shape'] = float64(input_img_shape)
            self.klparam_dict['numbasis'] = float64(numbasis)
            self.klparam_dict['output_imgs_shape'] = float64(
                output_img_shape)
            ...
            self.klparam_dict['aligned_center_x'] = float64(ref_center[0])
            self.klparam_dict['aligned_center_y'] = float64(ref_center[1])

            # We save information about the dataset that will be used when we load the KL basis
            self.klparam_dict['PAs'] = float64(self.PAs)
            self.klparam_dict['wvs'] = float64(self.wvs)

            self.klparam_dict['nwvs'] = float64(self.nwvs)
            self.klparam_dict['nfiles'] = float64(self.nfiles)
    ...
    def save_kl_basis(self):
        ...
        for key in self.section_ind_dict.keys():
            ...
            self.radstart_dict[key] = float64(self.radstart_dict[key])
            self.radend_dict[key] = float64(self.radend_dict[key])
            self.phistart_dict[key] = float64(self.phistart_dict[key])
            self.phiend_dict[key] = float64(self.phiend_dict[key])
...
def _recursively_save_dict_contents_to_group(h5file, path, dic):
    ...
    for key, item in dic.items():
        if isinstance(item, (np.ndarray, np.int64, float64, str, bytes)):
