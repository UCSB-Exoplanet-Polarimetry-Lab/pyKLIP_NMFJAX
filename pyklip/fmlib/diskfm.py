...
class DiskFM(NoFM):
    ...
    def __init__(
        ...
        if self.load_from_basis is True:  # We want to load the FM basis
            ...
            # (IWA, OWA, aligned_center) are identical to the one used used on the data when measuring the KL
    ...
    def update_disk(self, model_disk):
        ...
        if np.size(model_disk_shape) > 2:  # Then it's a multiWL model
            ...
            if n_model_wvs != n_wv_per_file:
                ...
                raise ValueError(
                    "Number of wls in disk model ({0}) don't match number of wls in the data ({1})".format(
                        n_model_wvs, n_wv_per_file
                    )
                )
    ...
    def fm_from_eigen(
        ...
        **kwargs
        ...
        if self.aligned_center != ref_center:
            print(
                "The aligned_center for the model {0} and for the data {1} is different. Change and rerun".format(
                    self.aligned_center, ref_center
                )
            )
            raise ValueError(
                "The aligned_center for the model {0} and for the data {1} is different. Change and rerun".format(
    ...
    def save_kl_basis(self):
        ...
        if file_extension == ".pkl":
            ...
            raise ValueError(
                file_extension
                + " is not a possible extension. Filenames can haves 2 recognizable extension: .h5 or .pkl"
            )
    ...
    def load_basis_files(self, dataset):
        ...
        if file_extension == ".pkl":
            ...
            if sys.version_info.major == 3:
                ...
                self.section_ind_dict = pickle.load(pickle_file, encoding="latin1")

                self.radstart_dict = pickle.load(pickle_file, encoding="latin1")
                self.radend_dict = pickle.load(pickle_file, encoding="latin1")
                self.phistart_dict = pickle.load(pickle_file, encoding="latin1")
                self.phiend_dict = pickle.load(pickle_file, encoding="latin1")
                self.input_img_num_dict = pickle.load(pickle_file, encoding="latin1")
        ...
        perturbmag, perturbmag_shape = self.alloc_perturbmag(
            self.output_imgs_shape, self.numbasis
        )
    ...
    def fm_parallelized(self):
        ...
        perturbmag_np = fm._arraytonumpy(
            perturbmag, perturbmag_shape, dtype=self.data_type
        )
        ...
        for key in self.dict_keys:  ## loop pver the sections/images
            ...
            self.fm_from_eigen(
                ...
                pas=self.pa_imgs_np[ref_psfs_indicies],
                wvs=self.wvs_imgs_np[ref_psfs_indicies],
                radstart=radstart,
                radend=radend,
                phistart=phistart,
                phiend=phiend,
