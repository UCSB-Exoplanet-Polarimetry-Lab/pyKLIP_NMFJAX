...
class FMPlanetPSF(NoFM):
    ...
    def generate_models(self, input_img_shape, section_ind, pas, wvs, radstart, radend, phistart, phiend, padding, ref_center, parang, ref_wv, flipx):
        ...
        # establish bounds to create stamp for stellar psf
        ...
        # create a blank image (array) to write model PSFs into
        ...
        for pa, wv in zip(pas, wvs):
            ...
            x_vec_stamp_centered = np.copy(x_grid[0, (planet_centx-psf_left):(planet_centx+psf_right)]) - psf_centx
            y_vec_stamp_centered = np.copy(y_grid[(planet_centy-psf_lower):(planet_centy+psf_upper), 0]) - psf_centy
            ...
            # create star stamp with new, planet centered coordinates
            star_stamp_centered = whiteboard[(planet_centy-psf_lower):(planet_centy+psf_upper), (planet_centx-psf_left):(planet_centx+psf_right)]

            # calculate the distance form the center of the star in planet centered co-ordinates
            dx_star_center = x_grid[planet_centy-psf_lower):(planet_centy+psf_upper), (planet_centx-psf_left):(planet_centx+psf_right)]
            dy_star_center = y_grid[planet_centy-psf_lower):(planet_centy+psf_upper), (planet_centx-psf_left):(planet_centx+psf_right)]
            star_stamp_centered = \
                    self.field_dependent_correction(whiteboard[star_stamp_centered], dx_star_center, dy_star_center)
