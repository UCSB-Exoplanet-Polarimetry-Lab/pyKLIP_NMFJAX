...
def test_matern32_cython():
    """
    Tests the Matern Kernel (cython implementation)
    """
    x = np.arange(10.)
    ...
    # test cython version/default version
    cov = covars.matern32(x, y, sigmas, corr_len, mode='cython')
...
def test_matern32_numpy():
    ...
    # test numpy version
    cov = covars.matern32(x, y, sigmas, corr_len, mode='numpy')
...
def test_sq_exp_cython():
    """
    Tests the Square Exponential Kernel (cython implementation)
    """
    x = np.arange(10.)
    y = np.arange(15., 15 + x.shape[0])
    sigmas = np.random.rand(x.shape[0])
    ...
    # test cython/default version
    cov = covars.sq_exp(x, y, sigmas, corr_len, mode='cython')
...
def test_sq_exp_numpy():
    ...
    # test numpy version
    cov = covars.sq_exp(x, y, sigmas, corr_len, mode='numpy')
